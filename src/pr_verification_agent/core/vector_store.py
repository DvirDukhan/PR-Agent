...
# For now, we'll use sentence-transformers directly since vectorizers module is not available
try:
...
class CodebaseVectorStore:
        ...
        if SENTENCE_TRANSFORMERS_AVAILABLE:
            try:
                self.vectorizer = SentenceTransformer(
                    self.config.repository.embedding_model
                )
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if not self.vectorizer and SENTENCE_TRANSFORMERS_AVAILABLE:
                try:
                    self.vectorizer = SentenceTransformer(
                        self.config.repository.embedding_model
                    )
            ...
            if self.use_async:
                self.index = AsyncSearchIndex(
                    schema=self._schema, redis_url=self.config.redis.connection_url
                )
            else:
    ...
    async def add_documents(self, documents: List[Dict[str, Any]]) -> List[str]:
        ...
        for doc in documents:
            ...
            if content and self.vectorizer:
                try:
